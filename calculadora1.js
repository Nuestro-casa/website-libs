      ...
      // Comienza calculadora
        ...
        */
        ...
        function calculate()
        {
          ...
          var inflation=(3.5/100) // C2
          var realAppreciation=(4/100) // C4
          var contractualAppreciation=(1.5/100) // C6
          var discount=(10/100) // C8
          var capRate=(10/100)  // C10
          var PrincipalSavingPremium=(2.6/100) // C12
          var fee=(1.2/100) // C14
          var canonExpenses=(12/100) // C16
          var accountingLegal=(1/100) // C18
          var sellCommission=(3/100) // C20
          var administration=(1.2/100) // C22
          var propertyInsurance=(0.2/100) // C24
          var propertyTax=(0.68/100) // C26
          var extraordinaryExpenses=(0.5/100) // C28
          var paymentsMonths=3 // C30
          var mortgageTerm=20 // C32
          ...
          var monthlyPaymentYearOne=1 // D42

          var OurfullRent=-price*(1-discount)*(capRate/12) // D59

          var OurexpensesOnCannon=-(canonExpenses+accountingLegal)*capRate*price*(1-discount)/12 // D60
          ...
          var OurFee=price*(1-discount)*(-fee)/12 //D61
            ...
            var rent=+OurfullRent*(1-initialFee/price)+(OurexpensesOnCannon+OurFee)*initialFee/price  // D58
...
////////////////////////////////////////////////////
           ...
           var annualNOI=+price*(1-discount)*(capRate*(1-canonExpenses-accountingLegal))*Math.pow((1+inflation),(monthlyPaymentYearOne-1)) // D65
           var housingExpenses=+price*(1-discount)*(-fee-propertyInsurance-propertyTax-extraordinaryExpenses) // D66
           var ownership=+initialFee/price // D56
           var savings=+(agreedAnnualSavings+(annualNOI+housingExpenses)*ownership)/12 // D63
          ...
          var expenses=(propertyInsurance2+propertyTax2+extraordinaryExpenses2)*ownership // D69
          ...
          */ 
          ...
          var interest=IPMT(rate,monthlyPaymentYearOne,mortgageTerm,(price-initialFee))/12 // D76
         ...
         var capitalContributions=+-+ExcelFormulas.PPMT(rate,monthlyPaymentYearOne,mortgageTerm,(price-initialFee))/12 // D75
         ...
         //fix results
          ...
          var interest2=-+IPMT(rate,monthlyPaymentYearOne,mortgageTerm,(price-initialFee))/12 // D82
          ...
          capitalContributions2=+-+ExcelFormulas.PPMT(rate,monthlyPaymentYearOne,mortgageTerm,(price-initialFee))/12
...
*/
     ...
     {
      var html = "<p>*este calculo tiene en cuenta un financiamiento de (x%) con una tasa y% a z años</p>";
    ...
    function createResult(data, data2)
    {
     ...
     var html = "<p>¿Cuanto vas a pagar al mes?</p><p>Con nuestro tu pago mensual es: $"+addCommas(result1)+"</p><p>Con la hipoteca(*) tu pago mensual es: $"+addCommas(result2)+"</p>"
...
*/
 ...
 function addCommas(x){
  ...
  var parts = x.toString().split(".");
    parts[0] = parts[0].replace(/^0+/, '').replace(/\D/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    return parts.join(".");
   ...
   function validatePrice()
    ...
    function myalert()
    {
          ...
          if(initialFee < porcentage)
          {
            $("#myalert").html("Recuerda, en Colombia los bancos exigen una cuota inicial del 33% mas los costos de escrituración, Necesitas  $"+addCommas(porcentage)+" ahorrados")
          }
